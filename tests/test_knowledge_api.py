...
class TestKnowledgeAskAPI:
    """POST /knowledge/ask — 知识库问答"""

    _kb_name: str = ""

    @pytest.fixture(scope="class", autouse=True)
    def setup_kb(self, check_service_running):
        """创建临时知识库，测试结束后清理"""
        kb = f"test_ask_{_TS}"
        resp = requests.post(
            KNOWLEDGE_BASE_URL,
            data={"model_name": TEST_EMBED_MODEL, "db_name": kb},
        )
        if resp.status_code == 200 and resp.json().get("success"):
            TestKnowledgeAskAPI._kb_name = kb
            yield
            # 清理
            requests.delete(f"{KNOWLEDGE_BASE_URL}/{kb}")
        else:
            pytest.skip(f"无法创建测试知识库，跳过问答测试: {resp.text}")
            yield

    def test_ask_missing_query(self):
        """POST /knowledge/ask — 缺少 query 应返回错误"""
        logger.info("test_ask_missing_query")
        ...
        resp = requests.post(
            ...
            json={
                "prompt": "你是一个助手",
                "query": "",
                "db_name": TestKnowledgeAskAPI._kb_name,
            },
        )
        # 可能是 422（pydantic 必填校验）或业务层 4000
        assert resp.status_code in (200, 422)
        logger.info(f"✅ 正确拒绝空 query: status={resp.status_code}")

    def test_ask_nonexistent_db(self):
        """POST /knowledge/ask — 使用不存在的知识库应返回 4004"""
        logger.info("test_ask_nonexistent_db")
        ...
        resp = requests.post(
            ...
            json={
                "prompt": "你是一个助手",
                "query": "什么是 GIS？",
                "db_name": "nonexistent_kb_xyz_999",
            },
        )
        body = _assert_standard_response(resp, expect_success=False)
        assert body["code"] == 4004
        logger.info(f"✅ 正确返回 4004: {body['message']}")

    def test_ask_missing_required_fields(self):
        """POST /knowledge/ask — 缺少必填字段（prompt/db_name）应返回错误"""
        logger.info("test_ask_missing_required_fields")
        ...
        assert resp.status_code in (200, 422)
        logger.info(f"✅ 正确拒绝缺少必填字段: status={resp.status_code}")
