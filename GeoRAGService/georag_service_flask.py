...
class GeoRAGService:
    ...
    def create_database(self, request):
        ...
        if model_name not in self.get_available_embedding_models():
            return (
                jsonify({"error": f"Embedding model '{model_name}' is not available"}),
                400,
            )
    ...
    def delete_db(self, db_name):
        ...
        try:
            ...
            if success:
                return (
                    jsonify({"message": f"Database '{db_name}' deleted successfully"}),
                    200,
                )
            else:
                return jsonify({"error": f"Database '{db_name}' not found"}), 404
    ...
    def delete_document(self, filename):
        ...
        try:
            ...
            if os.path.exists(file_path):
                os.remove(file_path)
                return (
                    jsonify({"message": f"Document '{filename}' deleted successfully"}),
                    200,
                )
            else:
    ...
    def ask_question(self, request):
        ...
        if chat_model_name not in self.get_available_chat_models():
            return (
                jsonify({"error": f"Chat model '{chat_model_name}' is not available"}),
                400,
            )
                    ...
                    jsonify(
                        {
                            ...
                            "message_count": self.chat_sessions[session_id][
                                "message_count"
                            ],
                        }
                    ),
