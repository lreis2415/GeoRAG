...
class StandardResponse(Generic[T]):
    ...
    def __init__(
        self, success: bool, code: int, message: str, data: Optional[T] = None
    ):
...
async def value_error_exception_handler(request: Request, exc: ValueError):
    """处理ValueError异常"""
    return JSONResponse(
        status_code=400, content=error_response(message=str(exc), code=4000)
    )
...
async def start():
    ...
    return success_response(
        data={"status": "running", "version": "1.0.0"}, message="GeoRAG服务正常运行"
    )
...
async def ask_question(request: AskRequest):
    ...
    """
    try:
        result = georag_service.ask_question(
            request.query, request.db_name, request.chat_model_name
        )
        return success_response(data=result)
    except ValueError as e:
        return error_response(message=str(e), code=4000)
